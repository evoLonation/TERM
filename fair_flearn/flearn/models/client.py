...
class Client(object):
    ...
    def __init__(self, id, group=None, train_data={'x': [], 'y': []}, eval_data={'x': [], 'y': []}, data_seed=1,
                 model=None):
        self.model = model
        self.id = id  # integer
        self.group = group
    ...
    def train_error_one_class(self):
        train_dat = {'x': [], 'y': []}
        for i in range(len(self.train_data['x'])):
            if self.train_data['x'][i][21] == 1:
                train_dat['x'].append(self.train_data['x'][i])
                train_dat['y'].append(self.train_data['y'][i])
        tot_correct, loss = self.model.test(train_dat)
        return tot_correct, len(train_dat['y'])
